...
def main() -> int:
    ...
    try:
        ...
        df = read_ledger_csv(args.input)
        ledger_result = normalize_ledger(df, ledger_columns, args.date_mode, args.date_format)
        ...
        vies_data = build_vies_data(
            ...
            declarer_id="",
            declarer_name=args.submitter_person,
            registered_vat=ledger_result.company_vat,
            registered_name=taxpayer_name,
            registered_address="",
        )

        run_dir = make_run_dir(args.output_root, ledger_result.company_vat, tax_period)
        shutil.copyfile(args.input, run_dir / "input_original.csv")
